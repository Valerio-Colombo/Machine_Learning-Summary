...
\begin{align*}
    P(w|t, \Phi, \sigma^2) & \propto \underbrace{\mathcal{N} (w|w_0, S_0)}_{\text{prior}} \underbrace{\mathcal{N} (t|\Phi w, \sigma^2I_N)}_{\text{likelihood}} = \mathcal{N} (w_N,S_N) \\
    w_N                    & = S_N \bigg( S_0^{-1}w_0 + \frac{\Phi^T t}{\sigma^2} \bigg)                                                                                               \\
    S_N^{-1}               & = S_0^{-1} + \frac{\Phi^T \Phi}{\sigma^2}
\end{align*}
$N$ stands for the number of data points or observations in the dataset, and it is used to denote that the posterior parameters \( S_N \) and \( w_N \) are derived after considering all \( N \) observations.
