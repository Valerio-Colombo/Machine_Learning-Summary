...
\begin{figure}[htbp]
    ...
    \end{subfigure}
\end{figure}
Some data points are closer to the boundary than the margin because we account for noise in the data (we give a slack to the margin).

\paragraph*{Bound on the loss function}
...
\paragraph{Solution technique} We have seen previously how we can solve the quadratic problem of finding the weights $\alpha$. There are more efficient way to calculate the weights. For example \textbf{SMO (Sequential Minimal Optimization)}.
